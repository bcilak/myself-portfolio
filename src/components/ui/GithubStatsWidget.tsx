    ...
    if (loading || !data) {
        return (
            <div className="glass-card rounded-xl p-6 flex items-center justify-center h-full min-h-[140px]">
                <div className="w-6 h-6 border-[3px] border-[#2d2d2d] border-t-[#ff4d4d] rounded-full animate-spin"></div>
            </div>
        );
    }

    return (
        <div className="glass-card p-6 glow-accent h-full rotate-[0.6deg]">
            <div className="absolute left-1/2 top-[-13px] h-7 w-28 -translate-x-1/2 -rotate-2 border border-black/20 bg-[#e5e0d8]/80" />
            <div className="flex items-center justify-between mb-4">
                <h3 className="text-slate-900 dark:text-slate-100 font-semibold text-xl flex items-center gap-2">
