...
export default function BlogClientPage() {
    ...
    const allCategories = [t("allCategories"), ...Array.from(new Set(getBlogPosts(locale).map((p) => p.category)))];
    const [searchQuery, setSearchQuery] = useState("");

    const filtered = getBlogPosts(locale).filter((post) => {
        const matchesCategory = activeCategory === t("allCategories") || post.category === activeCategory;
        const matchesSearch =
            searchQuery === "" ||
    ...
    return (
        <div className="pt-24">
            <div className="max-w-6xl mx-auto px-6 pb-24">
                <AnimatedSection>
                    <div className="mb-12">
                        <p className="text-cyan-400 text-sm font-medium uppercase tracking-widest mb-3">{t("writing")}</p>
